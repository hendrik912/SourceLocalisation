...
def concatenate_images(paths_X, paths_R, output_path, min_max_X=(0,1), min_max_R=(0,1), include_cb=True, offset=(100,100)):
    ...
    if include_cb:
        # concatenated_image.paste(img_cb_X, (x_pos + img_cb_X.width + offset[0]//downscale_factor, offset[1]//downscale_factor))
        # concatenated_image.paste(img_cb_R, (x_pos + img_cb_X.width, X_img.height + offset[1]//downscale_factor))
...
if __name__ == "__main__":
    ...
    b_source_localisation = True
    b_cut_images = True
    b_concat_images = True
    
    result_path = "results"
    methods = ['eLORETA'] # , 'eLORETA', 'dSPM', 'MNE']
    ds_name = "KUL"
    class_labels = [0,1]
    cluster_ids = [1,2]
    data_domains = ['X_time']
